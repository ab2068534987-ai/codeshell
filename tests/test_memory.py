...
class TestLoadInstructions:
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """对齐 Go 版发现顺序：CODESHELL.md 在前，.codeshell/INSTRUCTIONS.md（legacy）在后。"""
        root_md = tmp_path / "CODESHELL.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".codeshell"
        dotdir.mkdir()
        # Go 版不发现 .codeshell/CODESHELL.md，只发现 .codeshell/INSTRUCTIONS.md（legacy）
...
class TestSession:
    ...
    def test_update_context_usage_anchor_persists_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".codeshell" / "sessions"
        sessions_dir.mkdir(parents=True)
        meta = SessionMeta(id="test_session")
        jsonl_path = sessions_dir / "test_session.jsonl"
        ...
        conv.record_usage_anchor(input_tokens=10_000, output_tokens=500)

        with open(jsonl_path, "a", encoding="utf-8") as f:
            session = Session("test_session", f, meta, sessions_dir)
            session.update_context_usage_anchor(conv)

        loaded = SessionMeta.load(sessions_dir / "test_session.meta")
        assert loaded is not None
        assert loaded.context_baseline_tokens == 10_500
        assert loaded.context_anchor_count == 1
...
class TestSessionMeta:
    def test_save_and_load(self, tmp_path: Path) -> None:
        meta = SessionMeta(
            ...
            message_count=10,
            total_tokens=5000,
            context_baseline_tokens=12_345,
            context_anchor_count=7,
        )
        ...
        assert loaded.context_baseline_tokens == 12_345
        assert loaded.context_anchor_count == 7
