...
class TestUsageAnchor:
    ...
    def test_restore_usage_anchor_clamps_anchor_count(self) -> None:
        conv = ConversationManager()
        conv.add_user_message("a")
        conv.add_user_message("b")

        conv.restore_usage_anchor(12_000, 99)

        assert conv.baseline_tokens == 12_000
        assert conv.anchor_count == len(conv.history)
        assert conv.last_input_tokens == 12_000
        assert conv.current_tokens() == 12_000
