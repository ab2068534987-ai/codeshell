...
class TestSessionHandler:
    ...
    @pytest.mark.asyncio
    async def test_session_times_render_in_local_timezone(self) -> None:
        from codeshell.commands.handlers.session import handle_session
        ...
        meta = SessionMeta(
            ...
            title="test session",
            message_count=3,
            last_active=datetime(2026, 7, 8, 10, 11, tzinfo=timezone.utc),
        )
        sm = MagicMock()
        ...
        expected = meta.last_active.astimezone().strftime("%Y-%m-%d %H:%M")
        assert expected in ui.messages[0]
        assert "2026-07-08 10:11" not in ui.messages[0]

    @pytest.mark.asyncio
    async def test_session_resume_restores_context_usage_anchor(self) -> None:
        from codeshell.commands.handlers.session import handle_session
        from codeshell.memory.session import ResumeResult, SessionMeta

        ui = MockUI()
        meta = SessionMeta(
            id="session_test",
            context_baseline_tokens=22_000,
        ...
        sm = MagicMock()
        sm.resume.return_value = ResumeResult(
            session=session,
            messages=[Message(role="user", content="hello")],
            last_active=meta.last_active,
        )
        restored: list[ConversationManager] = []
        ctx = _make_context(args="resume session_test", ui=ui)
        ctx.session_manager = sm
        ctx.config["set_session"] = lambda s: None
        ctx.config["set_conversation"] = restored.append
        ctx.config["render_restored"] = AsyncMock()
        ctx.ui.refresh_status = MagicMock()

        await handle_session(ctx)

        assert restored
        assert restored[0].baseline_tokens == 22_000
        assert restored[0].anchor_count == 1
        assert restored[0].current_tokens() == 22_000

    @pytest.mark.asyncio
    async def test_session_resume_uses_total_tokens_for_legacy_meta_anchor(self) -> None:
        from codeshell.commands.handlers.session import handle_session
        from codeshell.memory.session import ResumeResult, SessionMeta

        ui = MockUI()
        meta = SessionMeta(id="legacy_session", total_tokens=18_000)
        session = MagicMock()
        session.meta = meta
        session.session_id = meta.id
        sm = MagicMock()
        ...
        restored: list[ConversationManager] = []
        ctx = _make_context(args="resume legacy_session", ui=ui)
        ctx.session_manager = sm
        ctx.config["set_session"] = lambda s: None
        ctx.config["set_conversation"] = restored.append
        ctx.config["render_restored"] = AsyncMock()
        ...
        assert restored
        assert restored[0].baseline_tokens == 18_000
        assert restored[0].anchor_count == 1
        assert restored[0].current_tokens() == 18_000
...
class TestRegisterAllCommands:
    def test_all_commands_registered(self) -> None:
        ...
        expected = {
            ...
            "sandbox", "rewind", "status", "skill", "doctor",
        }
