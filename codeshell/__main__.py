...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from codeshell.diff_review import DiffReviewManager, DiffReviewResponse
    ...
    diff_review_manager = DiffReviewManager(file_state_cache=state_cache)
    ...
    agent = Agent(
        ...
        hook_engine=hook_engine,
        diff_review_manager=diff_review_manager,
    )
    ...
    async for event in agent.run(conv):
        if isinstance(event, StreamText):
            ...
            formatted = format_applied_diff(event.change_set)
            if is_json:
                emit_json({
                    "type": "applied_diff",
                    "tool_name": event.tool_name,
                    "summary": event.change_set.summary_text(),
                    "diff": diff,
                    "formatted": formatted,
                })
            elif formatted:
            ...
            message = "Diff review required: interactive approval is not available in -p mode"
            if is_json:
                emit_json({"type": "diff_review_required", "message": message})
            else:
                print(message, file=sys.stderr, flush=True)
            event.future.set_result(DiffReviewResponse.DENY)
