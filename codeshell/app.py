...
from codeshell.cache import FileCache
...
class CodeShellApp(App):
    ...
    def __init__(
        ...
        self.diff_review_manager = None

    @staticmethod
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        t = RichText()
        icon_style = "bold color(220)"
        meta_style = "color(242)"
        dragon_lines = [
            ("        /\\      /\\        ", "CodeShell v0.1.0"),
            ("   ____/  \\____/  \\____   ", model),
            ("  /___   o  __  o   ___\\  ", work_dir),
            ("  \\___      __      ___/  ", ""),
            ("      \\__/||  ||\\__/      ", ""),
        ]
        for index, (icon, metadata) in enumerate(dragon_lines):
            t.append(icon, style=icon_style)
            if metadata:
                t.append(metadata, style=meta_style)
            if index < len(dragon_lines) - 1:
                t.append("\n")
        return t
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        write_tool = self.registry.get("WriteFile")
        state_cache = getattr(write_tool, "_state_cache", None) if write_tool else None
        self.diff_review_manager = DiffReviewManager(
        ...
        self.agent = Agent(
            ...
            diff_review_manager=self.diff_review_manager,
        )
        ...
        self._update_context_usage_indicator()
    ...
    def _get_context_usage(self) -> tuple[int, int]:
        if self.agent is None:
            return 0, 0
        return self.conversation.current_tokens(), self.agent.context_window

    def _update_context_usage_indicator(self) -> None:
        try:
            indicator = self.query_one("#context-usage", ContextUsageIndicator)
        except Exception:
            return
        used, total = self._get_context_usage()
        indicator.update_usage(used, total)
    ...
    def _persist_compact_boundary(self, notification: CompactNotification) -> None:
        ...
        self.session.update_context_usage_anchor(self.conversation)
    ...
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        ...
        if not text:
            return
        if self._submitting and not text.startswith("/"):
            return
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        ...
        try:
            async for event in self.agent.run(self.conversation):
                ...
                elif isinstance(event, DiffReviewRequest):
                    await self._handle_diff_review_request(event)


                elif isinstance(event, AppliedDiffEvent):
                    await self._handle_applied_diff_event(event)
    ...
    async def _handle_applied_diff_event(self, event: AppliedDiffEvent) -> None:
        from codeshell.diff_dialog import InlineAppliedDiffWidget

        chat = self.query_one("#chat-area", VerticalScroll)
        widget = InlineAppliedDiffWidget(event.change_set)
        await chat.mount(widget)
        self.call_after_refresh(chat.scroll_end, animate=False)

    async def _handle_diff_review_request(self, request: DiffReviewRequest) -> None:
        from codeshell.diff_dialog import InlineDiffReviewWidget

        chat = self.query_one("#chat-area", VerticalScroll)
        widget = InlineDiffReviewWidget(request.change_set)
        self._pending_diff_review_request = request
        await chat.mount(widget)
        self.call_after_refresh(chat.scroll_end, animate=False)
        try:
            self.query_one("#chat-input").disabled = True
    ...
    def on_inline_diff_review_widget_responded(
        self, event: "InlineDiffReviewWidget.Responded"
    ) -> None:
        from codeshell.diff_dialog import InlineDiffReviewWidget

        req = getattr(self, "_pending_diff_review_request", None)
        if req is not None:
            req.future.set_result(event.response)
            self._pending_diff_review_request = None
        try:
            widget = self.query_one("#diff-review-inline", InlineDiffReviewWidget)
            widget.remove()
        except Exception:
            pass
        try:
