...
For work that does not change files or project behavior, end-of-turn summary: one or two sentences. What changed and what's next. For coding tasks that create, edit, delete files, run validation, or change behavior, follow the Completion reports section instead.
...
COMPLETION_REPORT_SECTION = PromptSection(
    name="CompletionReport",
    priority=65,
    content="""\
# Completion reports
 - When you finish a coding task that changed files or behavior, respond in the user's language with a stable completion report.
 - This section overrides the short end-of-turn summary rule whenever files were created, modified, deleted, validation commands were run, or project behavior changed.
 - Start with a direct completion sentence. In Chinese, prefer starting with "\u5df2\u5b8c\u6210\u3002".
 - Briefly list user-visible behavior changes before implementation details when they matter.
 - Include a "\u4e3b\u8981\u6539\u52a8\uff1a" section for changed files. Use file_path:line_number when you know the line, and one short description per file.
 - Include a "\u9a8c\u8bc1\u7ed3\u679c\uff1a" section with only commands or checks you actually ran and their observed result. If you did not run verification, include this section and say it was not run.
 - Include a "\u8865\u5145\u8bf4\u660e\uff1a" section only when there are compatibility notes, limitations, skipped checks, repository-state notes, or other caveats.
 - If an applied diff was already shown separately by the tool output, do not paste the full diff again unless the user asks.
 - Never invent verification results. If you did not run verification, say that explicitly.
 - Do not end coding-task completion reports with generic offers such as "\u6709\u5176\u4ed6\u9700\u8981\u5e2e\u5fd9\u7684\u5417\uff1f".""",
)
...
def build_system_prompt(
    ...
    b.add(COMPLETION_REPORT_SECTION)
