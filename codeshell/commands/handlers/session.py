...
async def handle_session(ctx: CommandContext) -> None:
    ...
    if sub == "":
        if ctx.session:
            m = ctx.session.meta
            ts = _format_local_time(m.last_active)
    ...
    if sub == "list":
        ...
        baseline_tokens = result.session.meta.context_baseline_tokens
        anchor_count = result.session.meta.context_anchor_count
        if baseline_tokens <= 0 and result.session.meta.total_tokens > 0:
            baseline_tokens = result.session.meta.total_tokens
            anchor_count = len(conv.history)
        conv.restore_usage_anchor(baseline_tokens, anchor_count)
