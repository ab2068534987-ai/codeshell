...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".codeshell" / "config.yaml",
        cwd / ".codeshell" / "config.yaml",
        cwd / ".codeshell" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .codeshell/config.yaml "
            "in project or ~/.codeshell/config.yaml"
        )
    return merged
