...
class Agent:
    def __init__(
        ...
        self.diff_review_manager = diff_review_manager
        self._loop_count = 0
                ...
                if batch.concurrent and len(batch.calls) > 1:
                    ...
                    for tc in batch.calls:
                        ...
                        async for item in self._execute_tool(tc):
                            if isinstance(item, (PermissionRequest, DiffReviewRequest, AppliedDiffEvent)):
                                yield item
                            else:
    ...
    async def _execute_single_tool_direct(
        ...
        try:
            ...
            if self._should_use_diff_review(tc.tool_name, tc.arguments):
                result, change_set = await self._prepare_and_apply_diff_tool(tc.tool_name, params)
                if change_set is not None:
                    result = self._append_applied_diff_output(result, change_set)
            else:
    ...
    async def _prepare_and_apply_diff_tool(
        self, tool_name: str, params: Any
    ) -> tuple[ToolResult, ChangeSet | None]:
        change_set = await self.diff_review_manager.prepare(
            tool_name, params, self.work_dir
        )
        if change_set.is_preview_error:
            return ToolResult(
                output=change_set.error_message or "Diff preview failed",
                is_error=True,
            ), None
        if not change_set.has_changes():
            return ToolResult(
                output=change_set.stdout or "No file changes.",
                is_error=False,
            ), None
        result = self.diff_review_manager.apply(change_set)
        if result.is_error:
            return result, None
        return result, change_set

    def _append_applied_diff_output(
        self, result: ToolResult, change_set: ChangeSet
    ) -> ToolResult:
        ...
        return ToolResult(output=output, is_error=result.is_error)

    def _should_use_diff_review(self, tool_name: str, arguments: dict[str, Any]) -> bool:
        manager = self.diff_review_manager
        if manager is None or not manager.supports(tool_name):
            return False
        if self.permission_mode == PermissionMode.PLAN and tool_name in ("EditFile", "WriteFile"):
            file_path = str(arguments.get("file_path", ""))
            checker = self.permission_checker
            if checker is not None and checker._is_plan_file(file_path):
                return False
        return True
